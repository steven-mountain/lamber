...
export default function TemplateForms({
  ...
  useEffect(() => {
    ...
    const add = (
      fieldKey: string,
      value: unknown,
      apply: (nextValue: unknown) => void,
    ) => {
      bindings.push({
        fieldKey,
        value,
        valueType: getProjectPresetValueType(fieldKey),
        sourceType: getProjectPresetValueType(fieldKey) === "dictionary_value" ? "dictionary" : "from_project",
        apply,
      });
    };
        ...
        {selectedTemplate && selectedTemplate.includes('会审') && (
          <div className="bg-card border border-border rounded-xl p-6 shadow-sm">
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div className="flex flex-col gap-1 col-span-2">
                <div className="grid gap-3 md:grid-cols-[minmax(280px,2fr)_minmax(360px,5fr)]">
                  <div className="flex min-w-0 flex-col gap-1">
                    ...
                    <input type="text" name="gen_branch_name" {...getBind("gen_branch_name", "XXXX")} placeholder="分公司名称" className="bg-card border border-input px-3 py-2 rounded-md" />
                  </div>
                  <div className="flex min-w-0 flex-col gap-1">
                    <CommonPresetFieldHeader
                      fieldKey={PRESET_FIELD_KEYS.approvalBranchAttendees}
                      kind="short_value"
                      value={formData.gen_branch_attendees ?? ""}
                      onApply={(nextValue) => handleFieldChange("gen_branch_attendees", nextValue)}
                    >
                      分公司参会人员
                    </CommonPresetFieldHeader>
                    <input type="text" name="gen_branch_attendees" {...getBind("gen_branch_attendees")} placeholder="人员D、人员E" className="bg-card border border-input px-3 py-2 rounded-md" />
                  </div>
                </div>
              </div>
              <div className="flex flex-col gap-1 col-span-2">
              ...
              <div className="flex flex-col gap-1">
                <CommonPresetFieldHeader
                  ...
                  kind="short_value"
                  value={formData.gen_threeization ?? "本项目不涉及三化方案。"}
                  onApply={(nextValue) => handleFieldChange("gen_threeization", nextValue)}
                >
                  三化方案
                </CommonPresetFieldHeader>
              ...
              <div className="flex flex-col gap-1">
                <CommonPresetFieldHeader
                  ...
                  kind="text_snippet"
                  value={formData.gen_strategic_value ?? ""}
                  onApply={(nextValue) => handleFieldChange("gen_strategic_value", nextValue)}
                >
                  战略价值
                </CommonPresetFieldHeader>
              ...
              <div className="flex flex-col gap-1">
                <CommonPresetFieldHeader
                  ...
                  kind="text_snippet"
                  value={formData.gen_tech_conclusion ?? "方案可行同时能满足客户需求。"}
                  onApply={(nextValue) => handleFieldChange("gen_tech_conclusion", nextValue)}
                >
                  结论
                </CommonPresetFieldHeader>
              ...
              <div className="flex flex-col gap-1 col-span-2">
                <CommonPresetFieldHeader
                  ...
                  kind="text_snippet"
                  value={formData.gen_review_acc ?? "是，项目投入收入核算完整，各表填写准确"}
                  onApply={(nextValue) => handleFieldChange("gen_review_acc", nextValue)}
                >
                  项目评审表准确完整
                </CommonPresetFieldHeader>
              ...
              <div className="flex flex-col gap-1 col-span-2 mt-2">
                <CommonPresetLabelHeader labelClassName="text-sm font-bold text-foreground">
                  是否涉及单一来源
                </CommonPresetLabelHeader>
                <BusinessDictionarySelect
                  dictionaryKey="yes_no"
                  value={hasSingleSource ? "是" : "否"}
                  onChange={(nextValue) => setHasSingleSource(nextValue === "是")}
                  fallbackOptions={YES_NO_OPTIONS}
                  className="bg-card border border-input px-3 py-2 rounded-md"
                />
                {hasSingleSource && (
                  <div className="mt-2 flex flex-col gap-1">
                    <CommonPresetFieldHeader
                      ...
                      kind="text_snippet"
                      value={formData.gen_single_source ?? "单一来源决策依据：符合单一来源场景..."}
                      onApply={(nextValue) => handleFieldChange("gen_single_source", nextValue)}
                    >
                      单一来源决策依据
                    </CommonPresetFieldHeader>
                ...
                {procurementMethod === '其他' && (
                  <div className="mt-1 flex flex-col gap-1">
                    <CommonPresetFieldHeader
                      ...
                      kind="short_value"
                      value={formData.gen_procurement_method_other ?? ""}
                      onApply={(nextValue) => handleFieldChange("gen_procurement_method_other", nextValue)}
                    >
                      其他采购方式
                    </CommonPresetFieldHeader>
                    <input type="text" name="gen_procurement_method_other" {...getBind("gen_procurement_method_other")} placeholder="请输入其他采购方式" className="bg-card border border-input px-3 py-2 rounded-md" />
                  </div>
                )}
              </div>
              <div className="flex flex-col gap-1">
              ...
              <div className="flex flex-col gap-1 col-span-2">
                <CommonPresetFieldHeader
                  ...
                  kind="text_snippet"
                  value={formData.gen_construction_interface ?? "本项目采购统一集成单位实施。分公司负责客户侧的协调工作，并协调管理合作伙伴完成交付。"}
                  onApply={(nextValue) => handleFieldChange("gen_construction_interface", nextValue)}
                >
                  售中建设及施工界面
                </CommonPresetFieldHeader>
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div className="flex flex-col gap-1 col-span-2">
                <CommonPresetFieldHeader
                  ...
                  kind="text_snippet"
                  value={formData.gen_demand_device_list ?? "不涉及"}
                  onApply={(nextValue) => handleFieldChange("gen_demand_device_list", nextValue)}
                >
                  设备清单
                </CommonPresetFieldHeader>
                ...
                {hasSecurity && (
                  <div className="mt-2 flex flex-col gap-1">
                    <CommonPresetFieldHeader
                      ...
                      kind="text_snippet"
                      value={formData.gen_demand_security_detail ?? ""}
                      onApply={(nextValue) => handleFieldChange("gen_demand_security_detail", nextValue)}
                    >
                      信息安全及密评说明
                    </CommonPresetFieldHeader>
