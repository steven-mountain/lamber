...
impl BulkRelocationService {
    ...
    pub fn get_relocation_preview(
        ...
        for d in dir_iter {
            ...
            let old_path = Path::new(&old_root_path)
                .join(&relative_path)
                .to_string_lossy()
                .to_string();
    ...
    pub fn execute_bulk_relocation(
        &self,
        old_root_id: &str,
        new_root_path: &str,
    ) -> Result<(), String> {
        ...
        for (_id, project_id, relative_path) in dirs_to_update {
            let new_path = Path::new(new_root_path)
                .join(&relative_path)
                .to_string_lossy()
                .to_string();
            tx.execute(
        ...
        let mut stmt_files = tx
            .prepare("SELECT id, relative_path FROM project_files WHERE root_id = ?1")
            .map_err(|e| e.to_string())?;
