> ## ⛔ 已冻结 · 不要开工
>
> 当前唯一在做的是 **`docs/tasks/TASK_BOOK_dsh_full_integration.md`（dsh 完全融合 · 全面替换 AI 层）**。
> 本文排在它和会话绑定任务书之后，是第三顺位。
>
> 本文默认 dsh 已经是产品里能用的东西——**目前不是**：它只在开发机的仓库里、
> 通过隐藏路由 `#/agent-lab` 能跑，打包安装版里根本没有 dsh。前提没落地，本文的工具接不上去。

> **排序更新**：这份任务书排在 `docs/tasks/TASK_BOOK_cowork_session_project_binding.md`（Cowork 会话绑定项目 + 工具调用硬性限权）**之后**执行。本文里的工具都是"绑定会话里可用的工具"，前置的会话绑定/限权机制没做完之前不要开始这份。
