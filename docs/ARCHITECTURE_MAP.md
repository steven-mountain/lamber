...
graph TD
    ...
    Tauri -->|Data persistence| JSON[(projects_store.json)]
```
...
- **`src/benefit/`**: Benefit analysis engine.
  ...
  - [repository.rs](file:///d:/HermesJang/CMCC/tools/lambert/lamber/src-tauri/src/benefit/repository.rs): Handles JSON reads/writes via a shared store file.
...
- **`src/views/`**: Screen layouts.
  - [ProjectBoard.tsx](file:///d:/HermesJang/CMCC/tools/lambert/lamber/src-ui/src/views/ProjectBoard.tsx): Kanban lists and detail drawers.
...
3. The latest snapshot's `inputParams` are serialized and back-filled into React states when launching the calculator.
