...
- **`src/views/`**: Screen layouts.
  ...
  - [AiComputeQuoteView.tsx](../src-ui/src/features/ai-compute-quote/AiComputeQuoteView.tsx): Independent compute-quote blueprint page for project parameters, structured formulas, revenue/cost items, ICT subject output preview, and single-variable sensitivity analysis. It persists through project settings and does not write formal ICT state.
...
- **`src/features/ai-compute-quote/`**: 智算报价模块。`formulaEngine.ts` owns the Version 2 formula Token contract, legacy-formula conversion, safe recursive-descent parsing, business-name preview, and explicit formula errors without `eval`. `calculations.ts` owns dependency ordering, circular-reference isolation, quote summary, output aggregation, and sensitivity functions. `QuoteFormulaCalculator.tsx` is the expanded DIY editor, while expansion state remains local UI state. `store.ts` owns project-setting hydration and draft edits; the module reuses the ICT subject catalog but never writes `useIctState` or formal funding plans.
