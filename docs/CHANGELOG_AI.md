# 2026-09-10 · 完整官方 AI WebUI 与原生业务迁移

- 普通AI按钮直接启动dsh alpha.5官方WebUI；官方模块拥有会话、消息、流、工具、附件和设置框架。移除旧聊天容器及ACP显示双写，保留原业务服务和旧历史只读迁移。
- 通过独立Host适配限制会话/工作区/设置权限，生成字节一致的官方浏览器模块入口；实际Host集成检查有效部署，避免配置中同名行被误当作替换。
- 品牌/业务插槽复用可编辑审批、文档、清单、图片及D。操作账本固定身份、一次领取、持久回执；D确认仅改编辑器，手动保存和原财务公式/容差/计划保持。
- 原生工作区切换前拒绝挂起审批并回收Host；请求捕获原工作区审计记录器，专属缓冲仅回原工作区。深色业务面板使用官方主题token。开发入口单次生成Tauri Context，修复macOS dev链接失败。
- 正式隔离产品链路、冷恢复、零写拒绝/停止/关闭/切换与D独立对照已实跑；询价生成/编辑/截图保存与原页面一致，500px窄窗和系统图片粘贴已核对。Rust110通过（23 ignored），17组前端及lint/build、部署/打包检查通过；外部和未完成UI验收见[验证报告](./verification/ai-webui-upgrade.md)。未提交、发布或改真实项目数据。

## 2026-09-10 · 官方 WebUI 正式入口及业务适配（持续实施）

日常 AI 按钮接入 Rust 管理的官方 WebUI Host；新增可信会话准入、原审批可编辑视图复用、固定高层业务服务适配与单次领取回执账本。旧记录原文备份及后端可信映射恢复已实现；每轮业务上下文复用原序列化分层。修复设置保存后实例应用配置、未领取操作到期及开发 Context 重复展开问题。

隔离合成项目已实跑读取→编辑审批→原模板保存→原 Word 生成，并验证拒绝后版本不变。此轮为受控模型工程证据，完整卡片、历史桌面恢复、D 独立验收及安装/真实供应商验证仍待完成，不宣称整项升级已验收。

## 2026-09-10 · 修复 macOS 开发启动重复 Info.plist 符号

- Tauri Context 改由主入口单次生成，产品和官方WebUI探针按分支接收，消除两个宏展开引入的 `_EMBED_INFO_PLIST` 重定义。
- 实际 `npm run tauri dev` 编译并进入应用运行，默认feature普通构建通过；补充入口变更必须验证开发启动的长期规则，业务逻辑不变。

# 2026-09-10 · 官方 WebUI 第一批原生承载验证

- 锁定alpha.5，核对214个dsh包；新增debug隔离Tauri窗口和临时web profile验证脚本，官方完整界面通过品牌/字体插槽适配，未改node_modules或增加依赖。
- 受控SSE及真实模型中文/表格、停止续聊、原生宽窄窗通过；未登录API401、跨来源403。前端lint/build、Rust105通过（23 ignored未运行）及探针Node回归通过。
- 确定回环Host与同源官方Webview方向，列出身份、唯一审批、全局模型选择副作用、原生卡片服务和回执迁移边界。产品入口/业务工具/财务逻辑不变；阶段0仍待迁移契约实跑，未声明全UI升级或安装验收完成。[验证与接替表](./verification/ai-webui-upgrade.md)。
...
## 2026-09-10 — AI UI 整体升级任务书

用户确认完整采用 dsh 官方 WebUI，核心为 AI 界面升级。新增 [实施任务书](./tasks/TASK_BOOK_ai_webui_upgrade.md)，覆盖官方聊天体验、Lamber 主题与业务扩展、可信会话绑定、唯一审批决定、历史及回执迁移、旧界面收敛与安装验证。D 卡片保持确认只改编辑器、用户另行保存及独立验收；原财务规则与 AI 修图排除决定不变。

同步当前任务、任务索引、会话模块的待实施方向，并给旧 dsh 引擎融合任务书增加接续说明，避免早期事实误导后续执行。本轮仅更新文档，未改产品代码、运行配置或业务数据；不代表新版 UI 已实现或通过验收。


## 2026-09-10 — WebUI 视觉一致性任务书修订

用户反馈生产业务扩展与官方 WebUI 在圆角、填充色、位置上割裂。本轮仅修订文档：AI 窗口以同版本 dsh 原生组件与 token 为视觉基准，不强制沿用主应用 ROUND_FOUR 和独立蓝灰配色；其他产品页面不变。任务书新增第3.1节，记录4px圆角减为0px、双配色及裸历史按钮的源码路径，规定项目工具收纳、上下文语义、侧栏图标、全业务面板统一和整窗对照验收。同步 DESIGN、appearance 与 CURRENT_TASK；整改未实施、未验收，未改产品代码及业务数据。


## 2026-09-10 — 全产品设计基准改为 dsh WebUI

用户进一步明确：对齐方向是Lamber整体向dsh WebUI，而非仅AI窗口例外。此决定取代前条记录的“其他页面不变”视觉范围。DESIGN重写为全局dsh基准，AGENTS同步设计约束；appearance旧视觉段落标为迁移前实现。任务书第3.2节加入共享基础组件、项目、测算、模板、管理和设置的逐批对齐及跨窗口验收，任务索引和CURRENT_TASK同步。保留品牌、业务语义、财务精度与权限规则；仅更新文档，未改产品代码、用户配置或业务数据，未宣称视觉迁移完成。
