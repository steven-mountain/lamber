...
- **ICT Dynamic Reverse Subject Calculation**: Smart reverse calculation in `IctLifecycle.tsx` supports user-selected revenue or cost billing subjects through `ictReverseCalculation.ts`. Options come from `ICT_SUBJECT_DEFINITIONS` plus the current tax-item state and use stable refs (`side + subjectCode + groupId + key`), while labels reuse `璁¤垂绉戠洰鍚嶇О > 鍏蜂綋涓氬姟/浜у搧鍚嶇О > 鏍囧噯绉戠洰鍚嶇О`. The frontend solver no longer depends on the fixed `rev_it_integration` / `cost_it_integration` reverse commands for the smart reverse panel; it applies candidate inclusive amounts to the selected subject, invokes `calculate_ict_benefit`, and writes the final amount back through `updateTaxItem`. Only existing target metrics (`margin`, `npv_rate`) are supported.
- Dynamic reverse must respect balance allocation conflicts. Revenue and investment balance allocation are both currently implemented. When same-side balance allocation is valid, the balancing subject is disabled as a reverse target, while other same-side subjects enter locked-total structure reverse. The structure solver keeps the same-side inclusive total unchanged by changing the selected target subject and the balancing subject inversely over the finite pool `total - fixedOther`. Cross-side reverse remains allowed.
