...
> [!IMPORTANT]
> 本项目的 AI 上下文读取规则已调整为按需读取。关于初始加载和后续按需加载顺序，请严格遵循 [PROJECT_INDEX.md](./PROJECT_INDEX.md) 与 [AGENTS.md](../AGENTS.md) 的规定。
...
- **ICT Balance Allocation Rules**: Revenue and investment measurement pages support independent total-inclusive-amount balancing rules. The configuration (assigning a subject as the balancing subject role) is done directly on the concrete subject row via `SubjectRoleActions`. The frontend stores `balanceAllocation.revenue` and `balanceAllocation.investment`, serializes them as `revenue_balance_rule` / `investment_balance_rule` in lifecycle input payloads, and stores the current-state copy in cashflow assumptions as `balanceAllocation`. A configured total or balancing subject activates the rule. The revenue control area displays the own-product minimum prompt as 1% of the revenue total, while concrete amounts remain edited in the subject table. Difference calculation is inclusive amount based and applies only through the existing `updateTaxItem(..., "incl", amount)` path so tax-exclusive amounts and tax linkage remain centralized. The top control areas no longer render select dropdowns and instead display `SelectedSubjectRoleSummary` displaying the name of the assigned balancing subject, alongside a "定位" button (smooth-scrolls and highlights the subject element) and a "清除" button (clears the balancing role). The layout of this control area is optimized to be top-aligned (using `md:items-start` instead of `md:items-center`), and both columns' interactive second rows are matched to `38px` in height, ensuring Row 1 (labels), Row 2 (inputs/summaries), and Row 3 (1% own-product prompts) align row-by-row.
- Switching a configured balancing subject clears the previous balancing subject's inclusive amount to `0` before applying the current balancing difference to the newly selected subject, prompting the user for confirmation and preserving the old balancing subject's current amount value.
- **ICT Dynamic Reverse Subject Calculation**: Smart reverse calculation in `IctLifecycle.tsx` supports user-selected revenue or cost billing subjects through `ictReverseCalculation.ts`. The target reverse subject is selected directly on the subject row via `SubjectRoleActions` (and displays a `[反算目标]` badge). Options come from `ICT_SUBJECT_DEFINITIONS` plus the current tax-item state and use stable refs (`side + subjectCode + groupId + key`), while labels reuse `计费科目名称 > 具体业务/产品名称 > 标准科目名称`. The right smart reverse panel no longer has a side switcher or a subject select dropdown. It automatically reads the selected reverse target and its side. It shows a summary card of the active target (with locate/clear buttons), the inferred direction and reverse mode, and disables the "智能反算" button when no target is selected. The solver applies candidate inclusive amounts to the selected subject, invokes `calculate_ict_benefit`, and writes the final amount back through `updateTaxItem`. Only existing target metrics (`margin`, `npv_rate`) are supported.
...
- Locked-total structure reverse now synchronizes subject amounts into subject funding plans, not into formal `CashflowSegment` cashflow. `CashflowSegment` may still exist in legacy saved payloads, but it is no longer an official annual cashflow source.
- CT product and line revenue links continue to mirror into CT other-product and CT bandwidth cost subjects, and the paired amount changes flow through the same subject funding plan synchronization path.
- **ICT Subject-Level Funding Plans Final**: Each revenue/cost subject row maintains a collection/payment plan keyed by concrete subject instance `side + groupId + key` (`revenue:revIt:integration`, etc.). `src-ui/src/lib/ictSubjectFundingPlan.ts` owns the data model and pure helpers for `upfront`, `equal`, and `custom` 10-year tax-inclusive annual values, cents-exact validation, full source-coverage validation, normalization, upsert, cleanup, legacy migration, and annual subject-plan cashflow generation. Official calculation always uses `subject_funding_plans`; `legacy_model`, model A-E distributions, and `CashflowSegment` schedules are no longer user-selectable or formal cashflow branches. Old projects are migrated on load with `subjectFundingPlanMigrationVersion = 1`, generating first-year upfront migration plans for missing non-zero subjects while preserving existing valid plans. When coverage is valid, `useIctCalculations.ts` builds per-subject/per-year tax-exclusive annual arrays using each subject's own tax rate and passes them to Rust through `rev_cashflow_excl`, `cost_cashflow_excl`, `it_rev_cashflow_excl`, and `it_cost_cashflow_excl`. If coverage is invalid, the app keeps the last valid result visible, does not recalculate, does not fall back to legacy, and blocks benefit metric saves/document generation.
